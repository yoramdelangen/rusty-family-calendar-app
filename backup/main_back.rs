...
impl Node {
    ...
    fn width(mut self, w: Dimension) -> Self {
        self.style.width = w;
        self
    }
...
fn main() {
    ...
    div(NodeName::Header)
        ...
        .height(length(48.0))
        .border_b(5.0)
        .background_color(Color::from_rgba8(0, 0, 255, 100))
        .border_color(Color::from_rgba8(255, 0, 0, 100))
        .add(&mut layout);

    div(NodeName::Content)
        .width_full()
        .height_full()
        .background_color(Color::from_rgba8(0, 255, 0, 100))
        .add(&mut layout);
    ...
    div(NodeName::Footer)
        ...
        .height(length(48.0))
        .width_full()
        .background_color(Color::from_rgba8(245, 40, 145, 100))
        .add(&mut layout);

    let _ = grid("calendar".to_owned())
        .flex_direction(FlexDirection::Column)
        .flex_grow(1.0)
        .flex_shrink(1.0);

    let _ = NodeName::other("placeholder");
    let _unused_text_variant = NodeContent::Text(TextContext {
        content: String::new(),
        font_size: 16.0,
        color: Color::WHITE,
    });
